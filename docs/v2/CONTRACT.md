...
| 3 | nurb | the v2 engine importable from a pushed commit of this repo: `src/nurb` with `builder`, `checks`, `raster`, `printers.toml`, `tools.json` (redefined 2026-09-15; there is no PyPI release, nurb.app installs from the pinned commit) | — | shipped 2026-09-16: branch `v2` pushed, commit `9e93982` (version `1.0.0a1`, not merged to main while v1 is what people run); nurb.app pins that commit and bumps by hand |
...
| 5 | nurb | desktop rebuilt on `@nurb/ui`; Sign in, Publish, Back up, Pull, `nurb://open` handler | 1, 2, 3 | desktop half on the pushed `v2` branch (Phase 14, 2026-09-16); bridge half is Phase 15 |
